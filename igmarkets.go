...
func (ig *IGMarkets) ReadLiveStream(ctx context.Context, temp *[]Stock_local, ticker_ig string, ticker string) {
	for {
		...
		if error == nil {
			...
			*temp = append(*temp, temp_AAPL) //&temp_slice
		}
		time.Sleep(time.Second)
	}
}

func (ig *IGMarkets) ReadLiveStream2(ctx context.Context, epic string, ticker string, temp *[]MarketsResponse) {
	for {
		market, error := ig.GetMarkets(ctx, epic)
		if error != nil {
			println("There was an error when reading the market data")
			println(error)
			return
		} else {
			*temp = append(*temp, *market)
		}
		time.Sleep(time.Second)
	}
}
